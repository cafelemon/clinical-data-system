...
将研发侧需求正式并入系统，形成图像、增强图像、医生批注、JSON 字典、病灶标注的统一管理能力。
...
| -------- | ---- | -------------- |
| 原始图像     | 读    | 不直接写改        |
| 增强图像     | 读写   | 拷贝原始图像做副本后做增强，可读写，必须版本化          |
| 医生批注文档   | 读   | 必须保留来源和版本      |
| 元数据 JSON | 读   | 系统生成 + 临床人工补充，拷贝副本后研发可读写，版本化    |
| 标注 JSON  | 读   | 必须有标注人、时间、复核状态，拷贝副本后研发可读写 |
| 临床原始文档   | 按需只读 | 临床主责           |
