...
DOCUMENT_RULES: tuple[DocumentRule, ...] = (
    ...
    DocumentRule(
        doc_type="colonoscopy_report",
        display_name="结肠镜检查",
        target_code="结肠镜检查",
        title_terms=("结肠镜检查", "结肠镜检查报告", "电子结肠镜检查报告", "肠镜检查报告"),
        feature_terms=("结肠镜", "肠镜", "检查所见", "报告医师", "退镜", "息肉"),
        negative_terms=("胶囊内镜",),
        priority=87,
    ),
)

RULE_SUBJECT_ITEM_ALIASES: dict[str, tuple[str, ...]] = {
    "consent": ("知情同意书",),
    "consent_transfer": ("知情同意书交接表", "知情同意书交接表（若有）"),
    "ct_report": ("CT检查报告", "CT报告"),
    "his_record": ("HIS描述", "HIS记录"),
    "enrollment_review": ("入组审核记录表", "入组评估"),
    "vital_signs": ("生命体征记录", "生命体征记录表"),
    "comfort": ("舒适度评价表", "舒适度评价"),
    "capsule_endoscopy_report": (
        "结肠胶囊检查",
        "胶囊检查",
        "胶囊内镜报告",
        "胶囊内镜检查报告",
    ),
    "colonoscopy_report": ("结肠镜检查", "结肠镜检查报告", "胃肠镜检查报告", "肠镜检查报告"),
    "center_reading_quality": (
        "阅片",
        "中心阅片评价结果表",
        "独立评估人检查图像质量评估表",
        "图像质量评估",
    ),
    "image_quality": ("图像质量评价表", "图像质量评价", "阅片"),
    "device_secondary": (
        "性能评价",
        "设备常用功能评价",
        "设备稳定性评价",
    ...
    "other_secondary": ("性能评价", "其他次要指标评价表", "其他次要标准评价表"),
}
ALIAS_FIRST_DOC_TYPES = {
    "capsule_endoscopy_report",
    "colonoscopy_report",
    "center_reading_quality",
    "image_quality",
...
def candidate_match_score(
    candidate: SubjectItemLike,
    terms: tuple[str, ...],
) -> tuple[float, str] | None:
    normalized_name = normalize_for_match(candidate.item_name)
    normalized_code = normalize_for_match(candidate.item_code)
    keyword_values = [(keyword, normalize_for_match(keyword)) for keyword in candidate.keywords]
    best: tuple[float, str] | None = None
    for term_index, term in enumerate(terms):
        normalized_term = normalize_for_match(term)
        if not normalized_term:
            continue
        if normalized_term == normalized_name or normalized_term == normalized_code:
            score = 1.0
            reason = f"exact_alias={term}"
        elif normalized_term in normalized_name or normalized_name in normalized_term:
            score = 0.96
            reason = f"name_alias={term}"
        elif normalized_term in normalized_code or normalized_code in normalized_term:
            score = 0.93
            reason = f"code_alias={term}"
        else:
            keyword_hit = next(
                (
                    keyword
                    for keyword, normalized_keyword in keyword_values
                    if normalized_keyword
                    and (
                        normalized_term in normalized_keyword
                        or normalized_keyword in normalized_term
                    )
                ),
                None,
            )
            if keyword_hit is None:
                continue
            score = 0.9
            reason = f"keyword_alias={keyword_hit}"
        adjusted_score = score - (term_index * 0.01)
        if best is None or adjusted_score > best[0]:
            best = (adjusted_score, reason)
    return best


def remap_rule_to_subject_item(
    classification: PageClassification,
    candidates: list[SubjectItemLike],
) -> PageClassification | None:
    if not candidates or classification.doc_type is None:
        return classification
    alias_terms = RULE_SUBJECT_ITEM_ALIASES.get(classification.doc_type, ())
    rule_terms = tuple(
        term for term in (classification.target_code, classification.display_name) if term
    )
    if classification.doc_type in ALIAS_FIRST_DOC_TYPES:
        terms = (*alias_terms, *rule_terms)
    else:
        terms = (*rule_terms, *alias_terms)
    best: tuple[float, int, SubjectItemLike, str] | None = None
    for index, candidate in enumerate(candidates):
        match = candidate_match_score(candidate, terms)
        if match is None:
            continue
        score, reason = match
        if best is None or (score, -index) > (best[0], -best[1]):
            best = (score, index, candidate, reason)
    if best is None:
        return None
    score, _, candidate, reason = best
    confidence = round(max(classification.confidence, min(0.96, score)), 3)
    return PageClassification(
        **{
            **asdict(classification),
            ...
            "reason": (
                ...
                f"mapped_to_subject_item={candidate.item_name}({candidate.item_code}); {reason}"
            ),
        }
    )


def unmapped_rule_classification(classification: PageClassification) -> PageClassification:
    return PageClassification(
        **{
            **asdict(classification),
            "display_name": None,
            ...
            "reason": (
                f"{classification.reason}; unmapped_rule="
                f"{classification.display_name or classification.doc_type}; "
                "no current subject item match"
            ),
        }
    )
...
def classify_page(
    ...
    if rule_match is not None:
        remapped = remap_rule_to_subject_item(rule_match, candidates)
        if remapped is not None:
            return remapped
        return unmapped_rule_classification(rule_match)
