...
| V3.2.2 临床数据集 | 准备/进行/结束阶段中心级资料清单、SSU 进展维护、若有资料无材料声明口径、V1-V4 固定访视资料 | OCR 自动识别 SSU、OCR 自动切分试验进行阶段资料 |
| V3.2.3 看板 | 纯展示运营看板、看板维护独立入口、自动优先聚合接口、数据库字段设计文档 | 新增业务表、字段抽取落库 |
| V3.3.0 试验方案 | 项目级方案 PDF 版本、访视/资料项/中心解析草稿、人工修正、确认应用 | 大模型解析、自动删除旧配置、受试者自动创建 |
| V3.4.0 图像数据 | 按受试者管理原始图像、增强图像、电子报告，zip 安全统计，研发原始副本下载 | 单张图片预览、算法任务调度、自动复制研发副本 |
| V3.3 分类与布局 | 资料分类、板块体系、审核布局 | 具体字段抽取 |
...
| `TRIAL` | 试验进行阶段 | 资料准备 + V1-V4 固定访视阶段 |
...
- `V1_SCREENING_VISIT`、`V2_EXPERIMENTAL_FOLLOWUP_VISIT`、`V3_CONTROL_FOLLOWUP_VISIT`、`V4_UNSCHEDULED_VISIT`：试验实施访视阶段受试者资料。
...
- 受试者详情按 V1-V4 固定访视阶段展示资料项，实验组和对照组按对应访视阶段生成资料。
...
- V1-V4 访视阶段为系统固定结构，不作为普通阶段让用户手动增删改。
...
5. 用户在前端预览草稿中人工修正，确认应用后差异写入 `TRIAL` 下自定义访视、`subject_item` 资料模板和中心主数据。
...
- 默认不删除旧阶段、旧资料项和旧中心；停用/删除必须由后续显式能力处理。
- 本轮不接大模型，不做受试者自动创建，不改变临床数据集 materialize 逻辑。
...
- 本轮不做单图缩略图、单图预览、算法任务调度，也不创建独立研发副本记录。
...
- 受试者详情按 V1-V4 固定访视阶段展示资料项，旧试验阶段迁移后不再作为主结构出现。
